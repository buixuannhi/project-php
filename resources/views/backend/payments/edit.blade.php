                            ...
                            <div class="col-lg-4">
                                <div class="form-group">
                                    <input type="text" class="form-control" name="name" id="name"
                                        placeholder="Enter payments name ..."
                                        value="{{ request()->name ?? $payment_update->name }}">
                                </div>
                            </div>
                            <div class="form-group col-md-3">
                                <select name="status" class="form-control">
                                    <option {{ (request()->status ?? $payment_update->status == 1) ? 'selected' : '' }}
                                        {{ request()->status == 1 ? 'selected' : '' }} value="1">
                                        Show</option>
                                    <option {{ (request()->status ?? $payment_update->status == 0) ? 'selected' : '' }}
                                        {{ request()->status == 0 ? 'selected' : '' }} value="0">
                                        Hide</option>
                                </select>
                            </div>
                            <div class="col-md-2">
            ...
            <div class="col-lg-4">
                <div class="card card-primary">
                    <div class="px-4 py-2 bg-light border">
                        <h3 class="card-title my-2">Edit Payment</h3>
                    </div>
                    <form action="{{ route('payments.update', $payment_update->id) }}" method="POST"
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                                    name="name" value="{{ request()->name ?? $payment_update->name }}" autofocus>
                                @error('name')
                            ...
                            <select class="form-control" name="status" id="status">
                                <option {{ (old('status') ?? $payment_update->status == 1) ? 'selected' : '' }}
                                    value="1">
                                    Show
                                </option>
                                <option {{ (old('status') ?? $payment_update->status == 0) ? 'selected' : '' }}
                                    value="0">
                                    Hide
                                </option>
                            </select>
